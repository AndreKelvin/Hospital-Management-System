 ...
 * @author Andre Kelvin
 */
public class HospitalDoctorController implements Initializable /*extends MainStaff*/ {
...
//    }
    ...
    private String searchName,selectedDocName;
    private File docImageFile;
    private DeleteDoctorController del;
    private EditDoctorController edit;
    private AddDoctorController add;

    //Populate Search Auto Complete Textfield with Doctors Name
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            new File(System.getProperty("user.home") + File.separator + "DoctorImage").mkdir();
            
            //Pass Department Combo Box
            HospitalBridge.comboBoxDepartmentDoc=comboDept;
            ...
            //When Doctor Name is Selected thats when the Search Begins
            ...
            //Display Doctors Name on List View When Department is been Selected
            ...
            //Display Info When Doctor Name is been Selected
            listView.getSelectionModel().selectedItemProperty().addListener((ObservableValue observable, Object oldValue, Object newValue) -> {
                try {
                    if (newValue == null) {
                        ...
                        selectedDocName=newValue.toString();
                        
                        ps = HospitalDB.getCon().prepareStatement("Select * From Doctor Where Name=?");
                        ...
                        if (rs.next()) {
                            ...
                            docImageFile = new File(rs.getString("Photo"));
                            buff = ImageIO.read(docImageFile);
    ...
    private void displayDocName() {
        try {
            obList.clear();
            ps = HospitalDB.getCon().prepareStatement("Select Name From Doctor Where Department=?");
    ...
    /**
     * Set Department Combo Box Value To The Department Which The Doctors Name is been Searched for
     * This Will Automatically Populate the List View with Doctor Names in that Department
     * And Select The Actual Name That is been Searched for and Display the Info
     */
    private void searchAction() {
        try {
            searchName = textSearch.getText();
            ps = HospitalDB.getCon().prepareStatement("Select Department From Doctor Where Name=?");
    ...
    private void addAction() {
        try {
            if (loaderAdd == null) {
                loaderAdd = new FXMLLoader(getClass().getResource("AddDoctor.fxml"));
    ...
    private void editAction() {
        try {
            if (loaderEdit == null) {
                loaderEdit = new FXMLLoader(getClass().getResource("EditDoctor.fxml"));
                rootEdit = loaderEdit.load();
                edit=loaderEdit.getController();
            }
            edit.setComboBoxItems(comboDept.getItems());
            edit.setSelectedDocNameAndID(selectedDocName,Integer.parseInt(labelID.getText()));
            ...
            HospitalDialog.dialog.setOnDialogClosed((JFXDialogEvent event) -> {
                ...
                listView.getSelectionModel().select(selectedDocName);
    ...
    private void deleteAction() {
        try {
            if (loaderDelete == null) {
                loaderDelete = new FXMLLoader(getClass().getResource("DeleteDoctor.fxml"));
            ...
            del.getSelectedDocName(selectedDocName);
            del.getDocImageFile(docImageFile);
            del.getDocID(Integer.parseInt(labelID.getText()));
